...
mongoose.connect(
  "mongodb+srv://shashanky1502:shank@shank.synry.mongodb.net/bookyy?retryWrites=true&w=majority"
...
//GET  A SPECIFIC BOOK Loacalhost:3000/isbn
/*
 Route         /
...
Methods         GET
*/
booky.get("/c/:categoryy", (req,res) => {

    const getSpecificBook = database.books.filter(

        (book) => book.category.includes(req.params.categoryy)
    );

    if(getSpecificBook.length === 0){
        return res.json({
            error: `No book found for category of ${req.params.categoryy}`
        });
    }
...
booky.get("/lang/:langu", (req,res) => {

    const getSpecificBook = database.books.filter(
        (book) => book.language.includes(req.params.langu)
    );
...
booky.listen(3000,()=>console.log("server is up!!"));